...
export const metadata: Metadata = {
  ...
  description:
    "Download the PayVora app on Android and iOS. Coming soon to Google Play and the App Store.",
};

const features = [
  "Trade gift cards for instant cash",
  "Pay all your bills in seconds",
  "Get a free virtual USD card",
  "Recharge airtime & data cheaply",
        ...
        <div className="text-center mb-16">
          ...
          <div className="max-w-md mx-auto mb-20">
            <div className="bg-[#14141F] border border-[#2A2A3D] rounded-2xl p-6">
              <h2 className="text-white font-bold text-xl mb-2">Get notified at launch</h2>
              ...
              <form className="flex gap-3">
                ...
                <button
                  type="submit"
                  className="px-5 py-3 bg-[#00D9A0] text-[#0A0A0F] text-sm font-bold rounded-xl hover:bg-[#00C490] transition-colors whitespace-nowrap"
                >
                  Notify me
                </button>
              </form>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-left">
              {features.map((f) => (
                <div key={f} className="flex items-center gap-3 bg-[#14141F] border border-[#2A2A3D] rounded-xl p-4">
                  <CheckCircle2 size={18} className="text-[#00D9A0] flex-shrink-0" />
                  <span className="text-[#8F8FA3] text-sm">{f}</span>
                </div>
              ))}
            </div>
          </div>
