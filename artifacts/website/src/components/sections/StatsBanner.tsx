...
export function StatsBanner() {
  return (
    <section className="py-12 bg-[#F7F8FA] border-y border-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6 text-center">
          {stats.map(({ icon: Icon, value, label }) => (
            <div key={label} className="flex flex-col items-center gap-2">
              <div className="w-10 h-10 rounded-xl bg-[rgba(0,217,160,0.10)] flex items-center justify-center mb-1">
                <Icon size={18} className="text-[#00D9A0]" />
              </div>
              <p className="text-gray-900 text-xl font-black tracking-tight">{value}</p>
